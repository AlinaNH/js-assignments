...
function roundToPowerOfTen(num, pow) {
    const smallerMultiple = (pow === 0)
                ? num / 10 * 10
                : Math.floor(num / Number(10 + "0".repeat(pow-1))) * Number(10 + "0".repeat(pow-1)),
          largerMultiple = (pow === 0)
                ? smallerMultiple + 10
                : smallerMultiple + Number(10 + "0".repeat(pow-1));
    
    return ((num - smallerMultiple) > (largerMultiple - num)) ? largerMultiple : smallerMultiple;
}
...
function toNumber(value, def) {
    return (value instanceof Number || +value === parseInt(value, 10)) ? value : def;
}
